...
class Transform:
    ...
    def __call__(self, **inputs):
        ...
        return tuple(out.get(v) for v in var_names)

    def _split_inputs(self, inputs):
        var_names = [k for k in inputs.keys() if k in self._valid_inputs]
        n = next((len(inputs[k]) for k in var_names if inputs[k] is not None), 0)
        expanded = {k: ([None] * n if inputs[k] is None else inputs[k]) for k in var_names}
        split_inputs = [{k: v for k, v in zip(var_names, vals)} for vals in zip(*[expanded[vn] for vn in var_names])]
