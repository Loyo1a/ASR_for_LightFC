...
class EnvSettings_ITP:
    def __init__(self, workspace_dir, data_dir, save_dir):
        ...
        self.uavtrack_path = os.path.join(data_dir, 'uavtrack')
        self.uav123_10fps_path = os.path.join(data_dir, 'uav123_10fps')
        self.uav20l_path = os.path.join(data_dir, 'uav20l')
...
def env_settings(env_num):
    env_module_name = 'lib.test.evaluation.local'
    try:
        env_module = importlib.import_module(env_module_name)
        return env_module.local_env_settings(env_num)
    except:
