...
class CenterPredictor(nn.Module, ):
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16, freeze_bn=False):
        ...
        #self.ca_ctr = FEB(inplanes,inplanes)
        #self.se_ctr = SE(inplanes, inplanes)
        ...
        #self.ca_offset = FEB(inplanes,inplanes)
        #self.se_offset = SE(inplanes, inplanes)
        ...
        #self.ca_size = FEB(inplanes,inplanes)
        #self.se_size = SE(inplanes, inplanes)
    ...
    def get_score_map(self, x):
        ...
        # ctr branch
        #ca_ctr = self.ca_ctr(x)
        #se_ctr = self.se_ctr(x)
        #x_dpm = self.DPM(se_ctr)
        ...
        # offset branch
        #x_offset = self.ca_offset(x)
        #x_offset = self.se_offset(x)
        ...
        # size branch
        #x_size = self.ca_size(x)
        #x_size = self.se_size(x)
...
class CoordAtt(nn.Module):
    def __init__(self, inp, oup, reduction=32):
        ...
        # 中间层通道数，确保不低于8
        mid_channels = max(8, inp // reduction)

        self.pool_h = nn.AdaptiveAvgPool2d((None, 1))
        self.pool_w = nn.AdaptiveAvgPool2d((1, None))

        self.conv1 = nn.Conv2d(inp, mid_channels, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(mid_channels)
        self.act = nn.ReLU(inplace=True)  # 也可以使用 h_swish()

        self.conv_h = nn.Conv2d(mid_channels, oup, kernel_size=1, bias=False)
        self.conv_w = nn.Conv2d(mid_channels, oup, kernel_size=1, bias=False)
...
class FEB(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, reduction=4):
        ...
        self.att = CoordAtt(out_channels, out_channels, reduction=reduction)
