...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='emptytrack', help='training script name')
    parser.add_argument('--config', type=str, default='detection_datasets', help='yaml configure file name')
...
def main():
    ...
    if args.mode == "single":
        train_cmd = "python lib/train/run_training.py --script %s --config %s --save_dir %s " \
                    "--use_lmdb %d --script_prv %s --config_prv %s --distill %d --script_teacher %s " \
                    "--config_teacher %s --use_wandb %d --env_num %d" \
        ...
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d --master_port %d " \
                    "lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
                    "--script_prv %s --config_prv %s --use_wandb %d --env_num %d " \
                    ...
                    % (args.nproc_per_node, random.randint(10000, 50000), args.script, args.config, args.save_dir,
                       args.use_lmdb, args.script_prv, args.config_prv, args.use_wandb, args.env_num,
        ...
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d --master_addr %s --master_port %d " \
                    ...
                    "--use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d --env_num %d --distill %d " \
                    "--script_teacher %s --config_teacher %s" \
                    % (args.nproc_per_node, args.ip, args.port, args.world_size, args.rank, args.script, args.config,
                       args.save_dir, args.use_lmdb, args.script_prv, args.config_prv, args.use_wandb, args.env_num,
                       args.distill, args.script_teacher, args.config_teacher)
    else:
